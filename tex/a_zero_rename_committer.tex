...
\SetKwFunction{mergePathsV1}{mergePathsV1}
\SetKwFunction{mergePathsV2}{mergePathsV2}
...
\begin{procedure}
  ...
  \For{$f\leftarrow $ \listfiles(\localfs, \localAttemptPath) }{
...
\begin{procedure}
  \StagingVars
  \tcp{load the outstanding upload list from every committed task}
  $Pending \longleftarrow \emptyset$\;
  \For{$f \leftarrow $ \listfiles(\clusterfs, \clusterTaskAttemptPath)} {
    $Pending \longleftarrow Pending + $ \loadPendingSet(\clusterfs, $f$)\;
  }
  \checkForConflicts(\fs, $Pending$)
  \tcp{complete the outstanding uploads}
...
\begin{procedure}
  ...
  \magicPath $\longleftarrow$ \dest + \_\_$magic$\;
...
\begin{procedure}
  \MagicVars
  \tcp{List .pending files}
  $Pending \longleftarrow \emptyset$\;
  \For{$f \leftarrow $ \listfiles(\fs, \taskAttemptPath, $recursive$)}{
    $Pending \longleftarrow Pending + \{$ \loadPendingFile(\fs, $f$) $\}$\;
  }

  \tcp{Save the set as a .pendingset file for the job}
...
\begin{procedure}
  ...
  \tcp{load the outstanding upload list from every committed task}
  $Pending \longleftarrow \emptyset$\;
  \For{$f \leftarrow $ \listfiles(\fs, \jobAttemptPath)} {
  $Pending \longleftarrow Pending + $ \loadPendingSet(\clusterfs, $f$)\;
  }
  \tcp{complete the outstanding uploads}
  \For{$p \leftarrow Pending$}{
