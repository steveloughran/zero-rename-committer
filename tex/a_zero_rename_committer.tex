...
\subsubsection{Weaknesses of the Hadoop MapReduce commit algorithm}

Alongside some implementation details, such as the fact that a task process
will executed without calling cleanupTask() once it is informed that it
...
The committers themselves have to implement some mechanism;
the file committers do this through the filesystem, relying on consistent
...
a requirement met by POSIX-class filesytems, but not object stores.

Even with consistent with filesystems, the fact no direct list
of completed tasks is passed to the commitJob operation means that it is
not easy for the committer to determine whether the listing of tasks obtained
from the filesystem is complete.
If any generated files or directories were deleted between the completion
of a task and the job commit operation, this loss would be detected
Were to the Job Manager to pass its lists of completed tasks, distinguishing
those which generated data (i.e.\ requested permission to commit), from those
that did not, then output committers would have the ability to verify that
the committed tasks did generate output, while the uncommitted attempts did not.
...
\subsubsection{Weaknesses of the Spark commit algorithm}
...
Given that the filesystem metaphor is not viable, how can work safely
use S3 as a destination of work.

This is the problem solved by the new ``S3A committers'' so called as they
are designed to integrate with Hadoop's S3A connector to S3.
This connector was modified to offer access to an S3-specific operation:
multipart upload (MPU).

This the operation is already used for writing large files to the object store.
When a file is written, it is initially buffered to disk or memory, when it
reaches a configured threshold the upload is initiated, and first block uploaded
in a \texttt{POST} operation.
When that POST operation is finished, S3 returns an MD5-checksum of the
...
The ``Staging Commiter'' has each task attempt stage its data into the local
...
a shared consitent cluster filesystem (usually HDFS), \emph{using the v1
File Output Commit} commit algorith.
Thus the semantics of the v1 algorithm: atomic task attempt commit, are also
made available to the staging commmitter.
When the job is committed, Staging committer first commits the manifests
within the HDFS cluster, then reads in the final manifests, and completes
the multipart uploads.
All the data is uploaded to its final destination in the task commit, with the
...
Because of it support for incremental writes, the magic committer promises
faster uploads of larger datasets: there is no need to postpone the upload
to S3 until the task is actually committed.
However, it has two requirements.
Firstly, it needs consistent metadata listings of the object store, which
is provided by the S3Guard extension to S3A\ \cite{S3Guard}.
This uses Amazon's DynamoDB database for the consistent metadata view.
The second problem is: how to determine when a client wants to initiate
...
Accordingly, an MPU is initiated to
path calculated above the the \texttt{__magic} directory, and only the initial
manifest PUT under the \texttt{__magic} path, where it is then used in the
task- and job-commit operations.
Hence ``magic'' adjective: it modifies the filesystem so the files written
only appear when the job is committed, in a different location.
