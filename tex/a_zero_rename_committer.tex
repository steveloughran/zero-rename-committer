...
\textbf{Job}.
A parallelized query/operation to execute, composed of one or more Tasks.
...
\textbf{Task}.
Part of a job, such as a single Map or Reduce transformation of a fraction
of the input data.
A task completes successfully if it generates all the output it expects to without
failing in some way.

\textbf{Task Attempt}.
A single attempt to execute a task on a single process.
A task attempt is executed on a single host in a cluster;
...
determines when a job has been successful and publishes the results.
...
\textbf{Task Commit}.
The act of taking the output of a task attempt ---all files ---
...
preserving any hierarchy of directories.
...
\textbf{Independent.}
Individual tasks must be able to outpput without directly
...
\section{Hadoop's FileOutputCommitter}\label{sec:hadoop'sFileoutputcommitter}
...
\begin{table}
  ...
  \begin{tabular}{ l l l }
    ...
    $destDir$ & Job Destination Directory \\
...
The task of the commit algorithms, then, is to guarantee that this condition
...
must be unchanged.
Together, these non requirements imply that at-most-once semantics are required,
and that a higher-level workflow is expected to handle job failures.
...
\caption{setupJob()}
\label{FileOutputCommitter.setupJob}
\end{procedure}
...
A task attempt is committed simply by renaming the task attempt path
to the task committed path.
...
\eIf {\isFile(fs, src)} {
  ...
  \eIf {\exists(fs, dest)} {
    \eIf {\isFile(fs, dest)} {
      ...
      \For {f $\leftarrow$ \listFiles(fs, src)} {
...
% This is a bit more confusing than you think, as there are some bits of code
% which try to recover for both.
% look at TaskImpl.recover for how recovery is kicked off
...
The whole attempt recovery process is a complex one;
...
With the ability to recover from the failure of the entire job without losing
work means that jobs executed with this commit algorithm are
can recover faster from a failure of the job manager.
However, the (serialized) time to commit the job is longer, and the job
commit cannot be recovered from.

The longer lasting a job, the higher the probability of the Job Manager failing,
and the value in recovery higher.
Thus this commit strategy is most valuable in the larger, longer, jobs.
...
This is done with a in a variant \texttt{mergePaths()} algorithm, \ref{alg:mergePathsV2},
...
It has sacrificed resilience for speed.
...
Spark manages its requirement of ``only one task attempt may be commited''
...
A task attempt is only granted permission to commit iff three conditions
...
actively committing or has committed this task.
...
\SetKwData{clusterCommitter}{$clusterCommitter$}
...
\begin{procedure}
  ...
  \taskAttemptPath $\longleftarrow$ \jobAttemptPath/\taskAttemptId\;
  \clusterJobAttemptPath $\longleftarrow$ uniquePath(\localfs, \taskAttemptId)\;

  \clusterCommitter.setupJob(\clusterJobAttemptPath)\;
...
\begin{procedure}
  ...
  \clusterCommitter.setupTask(\clusterTaskAttemptPath)\;
...
volumem, along with a regurarly scheduled job to delete old temporary files.
...
A task is aborted by deleting all staged data, and aborting the cluster committer's
task.
...
\begin{procedure}
  ...
  \clusterCommitter.abortTask()\;

  \caption{StagingCommitter.abortTask()}
  \label{StagingCommitter.abortTask}
\end{procedure}
...
\subsection{The Magic Committer}\label{subsec:theMagicCommitter}
...
This will return true even if there are no .pending files under the task attempt
path.
A full path listing could determine this, but as this itself is potenally a slow
...
This file is then saved as a .pendingset file into the job attempt directory,
...
Because the .pendingset file is written in a single atomic PUT, the
commit of an individual task attempt is atomic.

If there are no .pending files, the saved .pendingset file will contain an empty
list of pending uploads.


\textbf{Task Abort}

A task is aborted by listing all .pending files in the task attempt directory,
...
\begin{procedure}
  ...
  \label{MagicCommitter.abortTask}
\end{procedure}
...
No attempt is made here to list any .pending or .pendingset files.
The committer cannot rely on those to completely enumerate all uploaded files,
specifically those of failed task attempts, where the information about
the pending upload may not have been not saved.
...
The Magic committer was written and tested with the expecation that either the
...
\begin{enumerate}
  ...
  \item A partitioned task attempt's uploads must become visible, even if uploaded after task or job commit
\end{enumerate}
...
Specifically, it exist immediately during the ping/response
