\documentclass[format=acmsmall, screen=true, nonacm, timestamp, review=false]{acmart}
...
\begin{itemize}
  ...
  \item If two tasks create files with the same name, the choice of which
  task's output is visible in the output is undefined.
  Rationale: nothing checks for this situation. The committers expect different
  tasks to create files with different names.
...
\begin{itemize}
    \item Spark added a random long value to the timestamp, reducing the probability of a collision.
    \item The removed option was reinstated in Spark.
    \item The committer code was reviewed to make sure all paths would use the \texttt{spark.sql.sources.writeJobUUID} option if supplied.
    \item A lot more logging about the origin of the unique ID was implemented, as well as control over the fallback process.
\end{itemize}

Fix #1 should suffice;
...
\subsubsection{Lack of reported progress on the magic committer}
...
The Magic committer's file output size isn't visible until job commit -and will be on a different path.
