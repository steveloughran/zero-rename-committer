...
What it does do is offer the atomic operations needed by MapReduce to safely use HDFS
...
\textbf{Task Attempt}.
A single attempt to complete a task on a single process.
A task attempt is executed on a single host in a cluster;
A task attempt is \emph{successful} if it generates all the output it expects to without
failing in some way.
...
sequentially, if addressing task failure, or in parallel when some
form of speculative taslk execution is taking place.
...
locations, including a YARN application master.
...
A second Job Manager process queries its Job Committer as to whether job
...
\TODO{v2 does attempt to recovery, though I'm not convinced its safe to do this}
...
If the number of skipped records in task attempt is below some threshold
...
The two are termed the ``v1'' and ``v2'' algorithms;
...
store will generate a false ``no data to commit'' negative result.
...
In a normal file system, the rename is an $O(1)$ atomic operation.
...
A Job can be cleaned up by deleting the output of all job attempts which
may have been made.
...
Unlike the Hadoop algorithm, there is no requirement for the Spark Driver
...
cluster fileystem as its $clusterJobAttemptPath$ output directory.
...
Task attempt code requesting a path for working data is returned a local
\texttt{file://}-prefixed directory, into which they save their output.
The task commit process then uploads these files as uncompleted-multipart uploads.
...
A task is aborted by deleting all staged data, and aborting the wrapped committer's
task.
...
The set of pending uploads are then saved as a manifest file to
\texttt{clusterTaskAttemptPath}, after which the wrapped committer has
...
only working withthe standard \texttt{FileOutputCommitter}, it was possible to declare
...
\footnote{the monotonically increasing YARN attempt ID value implicitly informs the Job whether or not it is the first attempt}.
...
never make changes to the visibile file set.
