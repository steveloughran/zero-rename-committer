...
sequentially, if addressing task failure, or in parallel when task attemps are
...
\label{subsec:commit_protocol_requirements}
...
\begin{enumerate}
  ...
  \item Support for thousands to tens of thousands of invidually scheduled $tasks$
  within a single $job$.
...
environment.
A central \emph{Resource Manager} manages the scheduing liveness monitoring
...
The YARN scheduler provides extra liveness monitoring: reporting exited Executors
to the Job Manager, and restarting the Job Manager itself on a failure.
...
YARN perceived Job Manager process to have failed, it instantiates
a new instance of the process somewhere within the cluster.
This new process creates an instance of the specififed Job Committer,
and queries it as to whether job recovery is supported.
If a committer does support recovery, it must recover tasks individually.
...
\label{subsec:hadoop_v1_commit_algorithm}
...
\TODO{v2 does attempt to do recovery, though I'm not convinced its safe to do this}.
...
This significantly improves time to execute a restarted job.
...
\label{subsec:hadoop_v2_commit_algorithm}
...
And it is potentially significantly more expensive in terms of RPC
...
Task commit is now a non-atomic recursive treewalk-and-rename operation.
It is therefore not possible to recover from the failure of a task to commit.
This also means that it's impossible to recover from the failure of a job other
than by deleting the destination directory and re-executing the entire job
\footnote{There is job recovery code, we just aren't convinced it works reliably}.
...
\label{subsec:limitations_of_hadoop_commit_protocol}
...
Even with consistent filesystems, the fact no direct list
...
from the filesystem is correct.
Were any generated files or directories to be added deleted between the completion
of a task and the job commit operation, these changes would be detected
...
\label{sec:spark_commit_protocol}
...
Operations upon directories are mimiced by listing all objects under that path,
...
\label{sec:improvements_to_commit_protocols}
